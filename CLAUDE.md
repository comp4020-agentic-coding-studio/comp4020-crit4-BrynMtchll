...
HTML/CSS/TypeScript-on-Vite to **Astro**, made in `comp4020-crit2`. Pages live
in `src/pages/` as `.astro` files (each one an entry, same "add it, link it"
model as the template's `.html` files); shared CSS lives in `src/styles/`.
Nothing in CI names a tool --- the whole contract, whatever stack a given week
uses, is:
...
`/comp4020-crit4-BrynMtchll/` explicitly --- Astro prefixes asset URLs it
